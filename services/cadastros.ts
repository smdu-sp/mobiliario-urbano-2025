        ...
        try {
            ...
            if (doc_especifica.length > 0)
                doc_especifica.map(async (file, index) => {
                    ...
                    if (!fs.existsSync(pasta)) fs.mkdirSync(`./uploads/${novo_cadastro.id}/doc_especifica`, { recursive: true })
                    const caminho = `${pasta}/${index + 1}.${ext}`
                    fs.writeFileSync(caminho, Buffer.from(data))
                    await tx.arquivo.create({ data: { caminho, cadastroId: novo_cadastro.id, tipo: "DOC_ESPECIFICA" }})
                })
            if (projetos.length > 0)
                projetos.map(async (file, index) => {
                    ...
                    if (!fs.existsSync(pasta)) fs.mkdirSync(`./uploads/${novo_cadastro.id}/projetos`, { recursive: true })
                    const caminho = `${pasta}/${index + 1}.${ext}`
                    fs.writeFileSync(caminho, Buffer.from(data))
